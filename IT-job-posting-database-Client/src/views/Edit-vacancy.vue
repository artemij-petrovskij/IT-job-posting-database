  ...
  <v-container>
    ...
    <v-fade-transition>
      <div v-if="show" class="my-box">
        <v-card title="Редактировать вакансию" class="mx-auto" max-width="900" variant="outlined">
          <v-container class="align-center text-center">
            <v-form ref="form1" v-model="valid" lazy-validation @submit.prevent="submitForm">
              ...
              <v-autocomplete disabled v-model="companyName" label="Компания*" :rules="[rules.required]"
              ...
              <v-autocomplete v-model="requirements" label="Опыт работы*" :rules="[rules.required]" :items="[
                'Не имеет значения',
                'Нет опыта',
                'От 1 года до 3 лет',
                'От 3 до 6 лет',
              ...
              {{ location }}
              <v-autocomplete v-model="location" label="Местанохождение" :rules="[rules.required]" :items="[
                'Минск',
                'Минская область',
                'Брест',
                'Витебск',
  ...
  methods: {
    ...
    async loadMyAdverts() {
      ...
      if (response.err) {
        ...
        let { title, description, salary, requirements, location, categoryId } = await Vacancy.getOneAdvert(id);
    ...
    async createAdv() {
      if (this.title != null && this.description != null) {
        ...
        let response = await Vacancy.editVacancy(data);
        ...
        if (response.err) {
          ...
          if (localStorage.getItem("jwt") != null) {
            if (this.$route.params.nextUrl != null) {
              ...
              this.$router.push("/vacancy-of-company");
            }
          }
        }
      }
